...
if $WATCH; then
  ...
  # Build Core
  BABEL_ENV=esm yarn babel src --out-dir lib --extensions '.ts' --only "$COREFILES" --source-maps
  # Modify file name
  find ./lib -name "*.js" -exec bash -c 'mv "$1" "${1/.js/.mjs}"' -- {} \;
  find ./lib -name "*.js.map" -exec bash -c 'mv "$1" "${1/.js.map/.mjs.map}"' -- {} \;
  # Build Core commonjs
  BABEL_ENV=browser yarn babel src --out-dir lib --extensions '.ts' --only "$COREFILES" --source-maps
  # Build Node
